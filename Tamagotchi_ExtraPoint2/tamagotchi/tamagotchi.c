...
int hours, minutes, seconds, sec_count, songnum, sound;
char age[50];
int barHappiness_count, barSatiety_count, offset;
volatile int is_eating = 0, is_cuddling = 0, eating_flag, left = 0;
...
void LCD_DrawHeart(uint16_t matrix[][N], int r, int c, int cord_x, int cord_y){
	int i, j;
	for(i = 0; i < r; i ++){
		for(j = 0; j < c; j ++){
			if(matrix[i][j] == 0)
...
//function which handles cuddles
void LCD_Cuddles(void){
